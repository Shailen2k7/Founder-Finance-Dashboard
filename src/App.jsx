...
function SmartImport({ accountId, entries, onDone, onClose }) {
  ...
  const [colMap,     setColMap]    = useState({ date:-1,desc:-1,debit:-1,credit:-1,amount:-1 });
  ...
  const buildPreview = useCallback(() => {
    if (!rawData) return;
    const existSet = new Set(entries.map(e => `${e.date}||${e.amount}||${(e.description||"").toLowerCase().slice(0,25)}`));
    const rows = rawData.rows.map((row, i) => {
      const dateStr  = colMap.date  >= 0 ? parseDate(row[colMap.date])   : null;
      const desc     = colMap.desc  >= 0 ? row[colMap.desc].trim()       : `Row ${i+1}`;
      const debitAmt = colMap.debit >= 0 ? parseAmount(row[colMap.debit]) : 0;
      const creditAmt= colMap.credit>= 0 ? parseAmount(row[colMap.credit]): 0;
      const singleAmt= colMap.amount>= 0 ? parseFloat(String(row[colMap.amount]).replace(/[₹£$,\s]/g,"")) || 0 : 0;
      ...
      const dupKey = `${dateStr}||${amount}||${desc.toLowerCase().slice(0,25)}`;
    ...
    setStep(3);
  }, [rawData, colMap, entries, accountId]);
  ...
  const doImport = async () => {
    ...
    for (const chunk of chunks) {
      const rows = chunk.map(r => ({ id: Date.now()+Math.floor(Math.random()*9999), date:r.date, business:r.business, account:r.account, type:r.type, category:r.category, description:r.description, amount:r.amount, source:"import" }));
    ...
    <div style={styles.overlay} onClick={e => e.target === e.currentTarget && onClose()}>
      <div style={styles.importModal}>
        ...
        <div style={styles.importHeader}>
          <div>
            <p style={{ margin:0, fontWeight:800, fontSize:18, color:"#111827" }}>
              📥 Smart Bank Import
            </p>
            <p style={{ margin:"2px 0 0", fontSize:13, color:"#6B7280" }}>
              {acct(accountId).label} · Supports CSV, Excel, XLSX
            </p>
          </div>
        ...
        <div style={{ display:"flex", gap:0, borderBottom:"1px solid #E5E7EB" }}>
          {["Upload","Map Columns","Preview & Edit","Done"].map((s,i) => (
        ...
        <div style={{ padding:"20px", overflowY:"auto", maxHeight:"calc(90vh - 140px)" }}>
          ...
          {step===1 && (
            <div>
              ...
              >
                ...
                <p style={{ fontWeight:700, fontSize:16, margin:"0 0 6px", color:"#374151" }}>
                ...
                <div style={{ display:"flex", gap:8, justifyContent:"center", flexWrap:"wrap" }}>
                  {["CSV","XLS","XLSX","ODS"].map(t => (
                    <span key={t} style={{ padding:"4px 10px", background:"#EEF2FF", borderRadius:8, fontSize:12, fontWeight:600, color:"#4F46E5" }}>{t}</span>
                  ))}
                </div>
                <input ref={fileRef} type="file" accept=".csv,.xlsx,.xls,.ods" style={{ display:"none" }} onChange={e=>handleFile(e.target.files[0])} />
              </div>
              {loading && <div style={{ textAlign:"center", marginTop:16, color:"#6B7280" }}>Parsing file…</div>}
              <div style={{ marginTop:20, padding:16, background:"#FFFBEB", borderRadius:12, border:"1px solid #FDE68A" }}>
                <p style={{ margin:"0 0 8px", fontWeight:700, fontSize:13, color:"#92400E" }}>💡 How to export your bank statement</p>
                <p style={{ margin:0, fontSize:13, color:"#78350F", lineHeight:1.6 }}>
                  <b>IDFC:</b> NetBanking → Accounts → Statement → Download CSV<br/>
                  <b>ICICI:</b> NetBanking → Accounts → View/Download Statement<br/>
                  <b>HDFC:</b> NetBanking → Statement of Account → Download<br/>
                  <b>Any bank:</b> Download as CSV or Excel — columns auto-detected
                </p>
              </div>
            </div>
          )}
          ...
          {step===2 && rawData && (
            <div>
              <p style={{ fontSize:14, color:"#374151", marginBottom:16 }}>
                <b>File:</b> {rawData.filename} · <b>{rawData.rows.length}</b> rows detected. Confirm column mapping:
              </p>
                ...
                {[
                  ...
                  <div key={key}>
                    ...
                    >
                      ...
                      {rawData.headers.map((h,i) => <option key={i} value={i}>{h || `Col ${i+1}`}</option>)}
                    </select>
                  </div>
                ))}
              </div>
              ...
              <div style={{ overflowX:"auto", marginBottom:16 }}>
                <table style={{ width:"100%", fontSize:11, borderCollapse:"collapse" }}>
                  <thead>
                    <tr>{rawData.headers.map((h,i)=><th key={i} style={{ ...styles.th, padding:"6px 8px", background:"#F3F4F6" }}>{h||`Col${i+1}`}</th>)}</tr>
                  </thead>
                  <tbody>
                    {rawData.rows.slice(0,3).map((row,i)=>(
                      <tr key={i}>{row.map((c,j)=><td key={j} style={{ ...styles.td, padding:"5px 8px", maxWidth:120, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{c}</td>)}</tr>
                    ))}
                  </tbody>
                </table>
              </div>
              <div style={{ display:"flex", gap:10 }}>
                ...
                <button onClick={buildPreview} style={styles.btnPrimary} disabled={colMap.date<0||colMap.desc<0}>
                  Preview {rawData.rows.length} rows →
                </button>
              </div>
            </div>
          )}
            ...
            <div>
              ...
              <div style={{ display:"flex", gap:10, marginBottom:12, flexWrap:"wrap", alignItems:"center" }}>
                <span style={{ ...styles.badge, background:"#DCFCE7", color:"#166534" }}>
                  ✅ {preview.filter(r=>!r.isDuplicate).length} new
                </span>
                <span style={{ ...styles.badge, background:"#FEF9C3", color:"#713F12" }}>
                  🔁 {preview.filter(r=>r.isDuplicate).length} duplicates
                </span>
              ...
              {/* Table */}
              <div style={{ overflowX:"auto", maxHeight:"45vh", overflowY:"auto", border:"1px solid #E5E7EB", borderRadius:10 }}>
                  ...
                  <tbody>
                    {displayed.map((row,i) => (
                      <tr key={row._id} style={{ background: row.isDuplicate ? "#FFFBEB" : i%2===0?"#fff":"#FAFAFA", opacity: row.isDuplicate?0.65:1 }}>
                        ...
                        <td style={{ ...styles.td, maxWidth:160, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }} title={row.description}>{row.description}</td>
                        <td style={styles.td}>
                          <span style={{ ...styles.badge, background:row.type==="Income"?"#DCFCE7":"#FEE2E2", color:row.type==="Income"?"#166534":"#7F1D1D" }}>{row.type}</span>
                        </td>
                        <td style={{ ...styles.td, fontSize:11, color:"#374151" }}>{row.category}</td>
                        <td style={styles.td}>{row.business}</td>
                        <td style={{ ...styles.td, fontWeight:700, color:row.type==="Income"?"#059669":"#DC2626", textAlign:"right" }}>
                          {row.isDuplicate && <span title="Possible duplicate" style={{ marginRight:4 }}>🔁</span>}
                          {fmt(row.amount)}
                        </td>
                        <td style={styles.td}>
                          <button onClick={()=>setEditRow({...row})} style={{ padding:"3px 8px", fontSize:11, background:"#EEF2FF", border:"none", borderRadius:6, cursor:"pointer", color:"#4F46E5" }}>Edit</button>
                        </td>
                      </tr>
                    ))}
                    {displayed.length===0 && (
                      <tr><td colSpan={8} style={{ ...styles.td, textAlign:"center", padding:40, color:"#9CA3AF" }}>No entries to show.</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
              ...
              <div style={{ display:"flex", gap:10, marginTop:16 }}>
                <button onClick={()=>setStep(2)} style={styles.btnGhost}>← Back</button>
          ...
          {step===4 && summary && (
            <div style={{ textAlign:"center", padding:"40px 20px" }}>
              ...
              <p style={{ fontSize:22, fontWeight:800, color:"#111827", margin:"0 0 8px" }}>Import Complete!</p>
              <div style={{ display:"flex", gap:12, justifyContent:"center", marginBottom:24, flexWrap:"wrap" }}>
                <div style={{ padding:"14px 24px", background:"#DCFCE7", borderRadius:12 }}>
                  <div style={{ fontSize:28, fontWeight:800, color:"#059669" }}>{summary.imported}</div>
                  <div style={{ fontSize:13, color:"#166534" }}>entries imported</div>
                </div>
                <div style={{ padding:"14px 24px", background:"#FEF9C3", borderRadius:12 }}>
                  <div style={{ fontSize:28, fontWeight:800, color:"#B45309" }}>{summary.skipped}</div>
                  <div style={{ fontSize:13, color:"#78350F" }}>duplicates skipped</div>
                </div>
              </div>
              <p style={{ fontSize:14, color:"#6B7280", marginBottom:20 }}>
                Dashboard has been updated automatically. Review imported entries in the Entries tab.
              </p>
              <button onClick={onClose} style={{ ...styles.btnPrimary, padding:"12px 32px" }}>Done ✓</button>
            </div>
          )}
        </div>
      </div>
      ...
      {editRow && (
        <div style={{ ...styles.overlay, zIndex:1100 }} onClick={e=>e.target===e.currentTarget&&setEditRow(null)}>
          <div style={{ background:"#fff", borderRadius:16, padding:20, width:"90%", maxWidth:420, boxShadow:"0 25px 60px rgba(0,0,0,0.2)" }}>
            ...
            <div style={{ display:"flex", gap:8 }}>
              ...
              <button onClick={()=>{
                setPreview(p=>p.map(r=>r._id===editRow._id?{...editRow}:r));
                setEditRow(null);
              }} style={styles.btnPrimary}>Save</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div style={{ background:"#fff", borderBottom:"1px solid #E5E7EB", position:"sticky", top:0, zIndex:100 }}>
        <div style={{ maxWidth:1440, margin:"0 auto", padding:"0 16px", display:"flex", alignItems:"center", gap:12, height:56 }}>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:6, fontSize:13, color:"#6B7280", flexShrink:0 }}>
            <span style={{ display:"none", "@media(min-width:640px)":{display:"inline"} }}>GBP→INR:</span>
            <input value={fxInput} onChange={e=>setFxInput(e.target.value)} onBlur={onFxBlur} style={{ width:64, padding:"4px 7px", border:"1px solid #BFDBFE", borderRadius:6, fontSize:13, outline:"none", background:"#EFF6FF", color:"#1D4ED8", fontWeight:600 }} />
          </div>
          <button onClick={()=>setShowAdd(true)} style={{ ...styles.btnPrimary, padding:"8px 14px", fontSize:13, flexShrink:0 }}>+ Add</button>
          {/* Mobile menu button */}
          <button onClick={()=>setMobileMenuOpen(v=>!v)} style={{ display:"none", padding:"8px", background:"none", border:"none", fontSize:20, cursor:"pointer", "@media(max-width:768px)":{display:"block"} }}>☰</button>
        </div>

        {/* ── Tab Nav (desktop) ── */}
        <div style={{ maxWidth:1440, margin:"0 auto", padding:"0 16px", display:"flex", overflowX:"auto", scrollbarWidth:"none" }}>
          {TABS.map(([id,lbl])=>(
            <button key={id} onClick={()=>setTab(id)} style={{ padding:"12px 16px", fontSize:13, fontWeight:tab===id?700:500, color:tab===id?"#4F46E5":"#4B5563", border:"none", borderBottom:tab===id?"3px solid #4F46E5":"3px solid transparent", marginBottom:-1, background:"none", cursor:"pointer", whiteSpace:"nowrap", flexShrink:0 }}>
              {lbl}
            </button>
          ))}
        </div>
